...
p = param_data();
figure(2)
t = tiledlayout(2, 2, TileSpacing="compact", Padding="compact") ;               % Set up the figure for plot
...
plot([min(param.AWinf.S), max(param.AWinf.S)], [23.4, 23.4], '-', 'Color', ...                                % 7.8774*param.AWinf.S.^0.114
...
set(gca, 'FontSize', 11, 'XTick', [10^(0) 10^(2) 10^(4)], 'Xscale', 'log', ...
    'Yscale', 'log', 'Box', 'on')                                                                                                                                                                                              
...
hold off
ylabel('Offspring mass, M_0 [g]')
title('B')
set(gca, 'FontSize', 11, 'XTick', [10^(1) 10^(3) 10^(5)], 'Xscale', 'log', ...
    'Yscale', 'log', 'Box', 'on')                                      
...
set(gca, 'FontSize', 11, 'XTick', [10^(-1) 10^(2) 10^(5)])
title('C')
box('on')
...
hold off
% % Create doublearrow
% annotation(gcf,'doublearrow',[0.685711805555556 0.685711805555556],...
%     [0.443685897435897 0.236089743589743],'LineWidth',1);
% 
% % Create textbox
...
ylim([0.000714432566659917 0.0153315]);

ylabel({'Minimum resource level', 'R^* [g]'})
set(gca, 'FontSize', 11, 'XTick', [10^(-1) 10^(2) 10^(5)])
legend('Teleost', 'Squid', 'Location', 'best')
title('D')
xlabel(t, 'Maximum mass, M_\infty [g]')
...
for dp = 1:length(depth)
    ...
    if dp ==1
        ...
        legend('Tot. With Squid', 'Tot. Without Squid' , 'Location', 'northwest', ...
...
for dp = 1:length(depth)
    ...
    hold off 
    if dp ==1
        ylabel({'Metabolic loss, Q_{tot} [g m^{-2} yr^{-1}]'})
    else 
        legend([param.SpId{:}, {'Total'}] , 'Location', 'best', ...
...
if Save_Figures
    save_graph(gcf, 'pdf', [ 'Fig/' 'SensAnalZoop'], 16, 13.5)
end 
...
if Save_Figures
    save_graph(gcf, 'pdf', [ 'Fig/' 'Ecosystem plots'], 16, 13.5)
end 
...
diet(50, 130, 1)
if Save_Figures
    save_graph(gcf, 'pdf', [ 'Fig/' 'feeding_level_shallow_squid'], 16, 5)
end 
...
diet(50, 130, 0)
if Save_Figures
    save_graph(gcf, 'pdf', [ 'Fig/' 'feeding_level_shallow_no_squid'], 16, 5)
end 
...
diet(2000, 130, 1)
if Save_Figures
    save_graph(gcf, 'pdf', [ 'Fig/' 'feeding_level_deep_squid'], 16, 5)
end 
...
diet(2000, 130, 0)
if Save_Figures
    save_graph(gcf, 'pdf', [ 'Fig/' 'feeding_level_deep_no_squid'], 16, 5)
end 
